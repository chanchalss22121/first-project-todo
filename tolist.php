		...
		<div class="container">
			...
			<?php
				include_once('logdb.php');
                  $a=1;
                  $stmt = $pdo->prepare("SELECT * FROM tbl_list");
                  $i = 1; 
				   $stmt->execute();
				...
				<div class="table2">
					...
					<div class=" listrow3 row">
						...
						$itm= $pdo->prepare("select * from tbl_item where list_id = $row[id] ORDER BY rank DESC ");
						$s=1;
						 ?>
					...
					<div class=" listrow2 row">
						...
						<div class="col-sm-9"><input type="text" id="item" name="item"class="item" required>
							<select name="list_id" id="listitem">
								<?php 
									$result = $pdo->prepare("SELECT * FROM tbl_list");
									 $result->execute();
